...
def upgrade() -> None:
    ...
    if 'calc_materials' not in existing_tables:
        op.create_table(
            'calc_materials',
    ...
    if 'calc_hardware' not in existing_tables:
        op.create_table(
            'calc_hardware',
            sa.Column('id', sa.Integer(), primary_key=True, index=True),
            sa.Column('name', sa.String(255), nullable=False),
    ...
    if 'calc_services' not in existing_tables:
        op.create_table(
            'calc_services',
            sa.Column('id', sa.Integer(), primary_key=True, index=True),
            sa.Column('name', sa.String(255), nullable=False),
    ...
    if 'calc_quotes' not in existing_tables:
        op.create_table(
            'calc_quotes',
            sa.Column('id', sa.Integer(), primary_key=True, index=True),
    ...
    # === Seed data (safely) ===
    op.execute("""
        INSERT INTO calc_materials (name, thickness_mm, price_per_m2, unit)
        SELECT name, thickness_mm, price_per_m2, unit FROM (
            VALUES 
        ...
        ) AS v(name, thickness_mm, price_per_m2, unit)
        WHERE NOT EXISTS (SELECT 1 FROM calc_materials WHERE name = v.name)
    """)

    op.execute("""
        INSERT INTO calc_hardware (name, brand, length_mm, category, price_per_unit, unit)
        SELECT name, brand, length_mm, category, price_per_unit, unit FROM (
            VALUES
        ...
        ) AS v(name, brand, length_mm, category, price_per_unit, unit)
        WHERE NOT EXISTS (SELECT 1 FROM calc_hardware WHERE name = v.name)
    """)

    op.execute("""
        INSERT INTO calc_services (name, price, unit)
        SELECT name, price, unit FROM (
            VALUES
        ...
        ) AS v(name, price, unit)
        WHERE NOT EXISTS (SELECT 1 FROM calc_services WHERE name = v.name)
    """)
