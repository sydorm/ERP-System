...
def upgrade() -> None:
    ...
    user_columns = [c['name'] for c in inspect_obj.get_columns('users')]
    ...
    if not inspect_obj.has_table('attendance_records'):
        op.create_table(
            'attendance_records',
    ...
    if not inspect_obj.has_table('payroll_transactions'):
        op.create_table(
