  ...
  <div class="page-container">
    <div class="page-header">
      <div class="header-left">
        ...
        <h2>{{ isEditMode ? 'Накладна №' + form.invoice_number : 'Нова видаткова накладна' }}</h2>
      </div>
      <div class="header-actions">
        <el-button @click="goBack">Скасувати</el-button>
        <el-button type="primary" :loading="submitting" @click="saveInvoice">
    ...
    <div class="editor-content" v-loading="loading">
      <el-form :model="form" label-position="top" class="invoice-form">
        <el-row :gutter="20">
          <el-col :span="6">
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item label="На підставі замовлення (опційно)">
              <el-select v-model="form.order_id" clearable placeholder="Оберіть замовлення" style="width: 100%" @change="loadFromOrder">
...
const loadFromOrder = async (orderId) => {
  ...
  try {
    ...
    form.counterparty_id = order.counterparty_id
    ...
    ElMessage.success('Дані замовлення завантажено')
  } catch (e) {
...
const fetchData = async () => {
  loading.value = true
  try {
    const [custRes, whRes, prodRes, ordRes] = await Promise.all([
      api.get('/api/v1/counterparties', { params: { is_customer: true } }),
      api.get('/api/v1/warehouses'),
      api.get('/api/v1/products'),
      api.get('/api/v1/orders', { params: { status: 'confirmed' } })
    ])
    ...
    if (isEditMode.value) {
      const res = await api.get(`/api/v1/sales-invoices/${route.params.id}`)
      Object.assign(form, res.data)
      if (typeof form.invoice_date === 'string') {
        form.invoice_date = new Date(form.invoice_date)
      }
    } else {
      addLine()
    }
...
const saveInvoice = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      await api.put(`/api/v1/sales-invoices/${route.params.id}`, payload)
      ElMessage.success('Накладну оновлено')
    } else {
      await api.post('/api/v1/sales-invoices', payload)
